...
class QdrantVectorIndex(BaseVectorIndex):
        ...
        for i, seg in enumerate(segments):
            ...
            points.append(
                PointStruct(
                    ...
                    % (
                        (2**63) - 1
                    ),  # Qdrant supports UUID strings, but integer hashes are safer for generic IDs unless we strictly enforce UUIDs. Let's use UUIDs or string IDs. Wait, Qdrant allows string UUIDs or integers. Let's just use string if it looks like UUID, else hash.
    ...
    def similarity_search(
        ...
        for hit in search_res:
            ...
            segments.append(
                TextSegment(
                    segment_id=str(hit.id), content=text, metadata=payload  # Qdrant IDs
                )
            )
