...
class MaticGraph:
    ...
    def _merge_state(self, current: Any, update: Any) -> Any:
        ...
        if isinstance(current, dict):
            for key, value in (
                update.items() if isinstance(update, dict) else vars(update).items()
            ):
                if key in current:
    ...
    def _execute_node(self, node_name: str, state: Any) -> Any:
        ...
        try:
            ...
            updated_state = (
                result if not self.stateful else self._merge_state(state, result)
            )
    ...
    def _execute_parallel_group(
        self, parallel_nodes: List[str], state: Any, verbose: bool = False
    ) -> Any:
        """
        ...
        with ThreadPoolExecutor(
            max_workers=min(self.max_workers, len(parallel_nodes))
        ) as executor:
            ...
            for future in as_completed(future_to_node):
                ...
                try:
                    ...
                    raise RuntimeError(
                        f"Parallel node '{node_name}' failed: {e}"
                    ) from e
        ...
        while current_node is not None and iteration < max_iterations:
            ...
            if isinstance(next_node, tuple) and next_node[0] == "PARALLEL":
                ...
                if join_node:
                    ...
                    if verbose:
                        print(
                            f"  [JOIN] Parallel execution complete, continuing to: {join_node}\n"
                        )
                else:
