...
class TabularIngestor:
    ...
    def ingest_file(self, source: str, table_name: Optional[str] = None, if_exists: str = "replace"):
        """
        ...
        try:
            if ext == ".csv":
                ...
                try:
                    ...
                    raise MissingDependencyError("openpyxl is required for Excel files. pip install openpyxl")
                df = pd.read_excel(source)
            elif ext == ".parquet":
                try:
                    import pyarrow  # noqa
                except ImportError:
                    raise MissingDependencyError("pyarrow is required for Parquet files. pip install pyarrow")
