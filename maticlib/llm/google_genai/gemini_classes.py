...
class GeminiResponse(LLMResponseBase):
    ...
    Attributes:
        responseId (str): Unique identifier for the Gemini response.
        modelVersion (str): Gemini model version.
        candidates (List[GeminiCandidate]): List of candidate responses.
        usageMetadata (GeminiUsageMetadata): Token usage metadata.
        cached_token_count (Optional[int]): Get cached content token count (Gemini context caching).
        thoughts_token_count (Optional[int]): Get the thoughts token count if available (Gemini-specific).
    """

    responseId: str = Field(
        ..., description="Unique identifier for the Gemini response"
    )
