...
def example_chroma_vectorstore():
    ...
    try:
        ...
        segments = [
            TextSegment(
                content="Apple pie is a delicious dessert.",
                segment_id="apple_1",
                metadata={"category": "food"},
            ),
            TextSegment(
                content="The new Macbook Pro has an M4 chip.",
                segment_id="apple_2",
                metadata={"category": "tech"},
            ),
            TextSegment(
                content="Bananas are rich in potassium.",
                segment_id="banana_1",
                metadata={"category": "food"},
            ),
        ]
        ...
        results = vector_index.similarity_search(
            query, k=2, filter_dict={"category": "tech"}
        )
        for r in results:
            print(f"  -> [{r.metadata.get('category')}] {r.content}")
        ...
        print(
            f"Example failed (Make sure you ran `pip install chromadb` and have an OpenAI key): {e}"
        )
