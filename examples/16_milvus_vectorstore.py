...
def example_milvus_vectorstore():
    ...
    try:
        ...
        vector_index = MilvusVectorIndex(
            embeddings=embeddings, collection_name="demo_collection", uri=db_file
        )

        # Add segments
        segments = [
            TextSegment(
                content="SpaceX launched a new rocket today.",
                segment_id="space_1",
                metadata={"topic": "space", "year": 2026},
            ),
            TextSegment(
                content="The new Mars rover found water.",
                segment_id="space_2",
                metadata={"topic": "space", "year": 2026},
            ),
            TextSegment(
                content="Local baker wins national pie contest.",
                segment_id="news_1",
                metadata={"topic": "local", "year": 2026},
            ),
        ]
        ...
        results = vector_index.similarity_search(
            query, k=2, filter_dict={"topic": "local"}
        )
        for r in results:
            print(f"  -> [{r.metadata.get('topic')}] {r.content}")
        ...
        print(
            f"Example failed (Make sure you ran `pip install pymilvus` and have an OpenAI key): {e}"
        )
