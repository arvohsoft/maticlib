...
def main():
    ...
    if os.getenv("OPENAI_API_KEY"):
        verify_provider("OpenAI (gpt-4o-mini)", OpenAIClient(model="gpt-4o-mini", verbose=False), query)
    else:
        print("\nSkipping OpenAI: OPENAI_API_KEY not set.")

    # 2. Google Gemini
    if os.getenv("GOOGLE_API_KEY") or os.getenv("GEMINI_API_KEY"):
        verify_provider("Google Gemini (gemini-2.5-flash-lite)", GoogleGenAIClient(model="gemini-2.5-flash-lite", verbose=False), query)
    else:
        print("\nSkipping Gemini: GOOGLE_API_KEY not set.")

    # 3. Mistral AI
    if os.getenv("MISTRAL_API_KEY"):
        verify_provider("Mistral AI (mistral-small-latest)", MistralClient(model="mistral-small-latest", verbose=False), query)
    else:
